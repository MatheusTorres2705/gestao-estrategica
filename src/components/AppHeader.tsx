...
export function AppHeader({ mes, ano, onMesChange, onAnoChange }: Props) {
  const today = new Date();
  const currentMes = today.getMonth() + 1;
  const currentAno = today.getFullYear();

  return (
    <header className="sticky top-0 z-20 flex items-center justify-between h-13 px-6 py-2.5 border-b border-gray-200 bg-white shadow-sm">
