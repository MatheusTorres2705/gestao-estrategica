...
async function sankhyaSave(jsessionid, entity, fields) {
  ...
  if (status && status !== '1') {
    const msg = r.data?.statusMessage || 'Erro retornado pelo Sankhya';
    throw new Error(msg);
  }
  return r.data;
}
...
app.post('/api/auth/refresh', (req, res) => {
  ...
  try {
    ...
    if (data.exp && now - data.exp > 7 * 24 * 3600) {
    ...
    const newToken = jwt.sign(
      { sub: data.sub, name: data.name, codusu: data.codusu, codvend: data.codvend, jti: newJti },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
...
// ====== ROTA OBTER REG (SQL query no Sankhya) ======
...
// ====== ROTA DATA SAVE (CRUD no Sankhya) ======
...
app.listen(PORT, () => {
  console.log(`🚀 Gestão Estratégica — servidor rodando em http://localhost:${PORT}`);
});
